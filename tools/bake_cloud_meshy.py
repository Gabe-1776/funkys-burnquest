"""Bake the Meshy cloud GLB to the game's JSON mesh format.

Run headless:
  /Applications/Blender.app/Contents/MacOS/Blender -b --python tools/bake_cloud_meshy.py

Imports test-shots/bakeoff-cloud/cloud_meshy.glb, joins to one object, converts
Blender Z-up to the game's Y-up, and emits the same compact JSON contract.
"""
import bpy, json, os

ROOT = os.path.expanduser("~/Developer/funkys-burnquest")
GLB = os.path.join(ROOT, "test-shots/bakeoff-cloud/cloud_meshy.glb")
OUT = os.path.join(ROOT, "assets/models/bakeoff/cloud_meshy.json")
...
def import_glb(path):
    ...
    for op in (
        ...
        try:
            ...
            print("  import attempt failed: %s" % e)
    if not ok:
...
obj = join_meshes(meshes, "CloudMeshy")
bake(obj, "cloud_meshy")
